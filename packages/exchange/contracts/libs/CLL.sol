...
library CLLu {
  ...
  function step(CLL storage self, uint n, bool d) internal view returns (uint) {
    return self.cll[n][d];
  }
  ...
  function seek(CLL storage self, uint a, uint b, bool d) internal view returns (uint r) {
    r = step(self, a, d);
